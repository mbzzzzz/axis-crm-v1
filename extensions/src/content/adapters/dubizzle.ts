...
async function fillDubizzleForm(payload: AutofillPayload) {
  console.log("AXIS Autofill: Starting Dubizzle form fill", payload.property);
  ...
  try {
    ...
    console.error("AXIS Autofill: Dubizzle form fill failed", error);
    throw error;
  }
}

const DubizzleAdapter: AutofillAdapter = {
  key: "dubizzle",
  matches: (location) => location.hostname.includes("dubizzle.com") || location.hostname.includes("dubizzle.ae"),
  apply: fillDubizzleForm,
};
