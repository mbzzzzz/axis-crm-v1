...
async function fillPropertyFinderForm(payload: AutofillPayload) {
  console.log("AXIS Autofill: Starting Property Finder form fill", payload.property);
  ...
  try {
    ...
    if (property.images?.length) {
      const uploadResult = await uploadImages('input[type="file"]', property.images, 'propertyfinder');
      if (uploadResult.success) {
        console.log(`AXIS Autofill: Successfully uploaded ${uploadResult.uploadedCount} images to Property Finder`);
      } else if (uploadResult.errors.length > 0) {
        console.warn(`AXIS Autofill: Image upload issues: ${uploadResult.errors.join(', ')}`);
      }
    }
    
    console.log("AXIS Autofill: Property Finder form fill completed successfully");
  } catch (error) {
    console.error("AXIS Autofill: Property Finder form fill failed", error);
    throw error;
  }
}

const PropertyFinderAdapter: AutofillAdapter = {
  key: "propertyfinder",
  matches: (location) => location.hostname.includes("propertyfinder.ae") || location.hostname.includes("propertyfinder.com"),
  apply: fillPropertyFinderForm,
};
