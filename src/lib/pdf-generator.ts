...
export function generateInvoicePDF(data: InvoiceData): jsPDF {
  ...
  const tableData = data.items.map(item => [
    ...
    formatCurrency(item.rate, currencyCode, { compact: false }),
    formatCurrency(item.amount, currencyCode, { compact: false })
  ]);
  ...
  doc.text(formatCurrency(data.subtotal, currencyCode, { compact: false }), pageWidth - 20, yPosition, { align: 'right' });

  yPosition += 7;
  doc.text(`Tax (${data.taxRate}%):`, totalsX, yPosition, { align: 'right' });
  doc.text(formatCurrency(data.taxAmount, currencyCode, { compact: false }), pageWidth - 20, yPosition, { align: 'right' });
  ...
  doc.setTextColor(...accentColor);
  doc.text(formatCurrency(data.totalAmount, currencyCode, { compact: false }), pageWidth - 20, yPosition, { align: 'right' });
