...
interface DownloadProps {
  ...
  activeText?: string; // NEW: allow custom active label (e.g., IMPORTING / EXPORTING)
}
...
export function AnimatedDownload({
  ...
  useEffect(() => {
    const newTargetText = isAnimating ? activeText.toUpperCase() : "READY";
    if (newTargetText !== targetText) {
      setTargetText(newTargetText);
      setTextIterations(0);
      setIsTextAnimating(true);
    }
  }, [isAnimating, targetText, activeText]);
